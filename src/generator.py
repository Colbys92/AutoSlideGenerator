...
def addImage(new_slide, slide_layout, slide):
    pic = ""
    if slide_layout == "title+paragraph+image":
        left = Inches(6)
        top = Inches(3)
        height = Inches(2.5)
        image_source_path = slide.xpath('image')[0].xpath('source')[0].text
        pic = new_slide.shapes.add_picture(image_source_path, left, top, height=height)
    return pic
...
def generatePresentation(dataPath, maskPath, export=True, exportPath="output.pptx"):
    ...
    for i, slide in enumerate(slides):
        ...
        # title and a small text paragraph layout
            ...
            new_slide = addSlide(presentation, layouts['only_title'])
            setSlideTitle(new_slide, slide_title)
            new_slide_textbox = addTextBox(new_slide, "title+paragraph")
            new_slide_textbox.text_frame.text = slide_paragraph
        ...
        # title and bullet and one image
            ...
            new_slide = addSlide(presentation, layouts['only_title'])
            setSlideTitle(new_slide, slide_title)
            new_slide_textbox = addTextBox(new_slide, "title+paragraph+image")
            new_slide_textbox.text_frame.text = slide_paragraph
